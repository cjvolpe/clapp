// builtin
import { ROPE_GRADES, BOULDER_GRADES } from "../../frontend/src/lib/types.js";
...
export function setupRoutes(server) {
    ...
    server.post("/climbs/search/filter", {
        config: {
            rateLimit: { max: 60, timeWindow: '1 minute' },
        }
    }, async (req, res) => {
    ...
    async function handleFeaturedClimbs() {
    ...
    async function handleGetClimbs() {
    ...
    async function handleNewClimb(req) {
    ...
    async function handleArchive(params) {
        const { id } = params;
    ...
    async function handleLoggedClimbs(params) {
        const { uuid } = params;
    ...
    async function handleFilteredSearch(req) {
        const { lowerDifficulty, upperDifficulty, type, color, startDate, endDate, gym, archived } = req;
        const query = server.supabase.from("climbs").select('*');
        let boulderList = Object.keys(BOULDER_GRADES);
        let ropeList = Object.keys(ROPE_GRADES);
        const filter = {
    ...
    function sortByGrade(type, bound, filterGrade, gradeList) {
        if (type === "Top Rope") {
            if (bound === "lower") {
    ...
    async function packageResponse(handler) {
